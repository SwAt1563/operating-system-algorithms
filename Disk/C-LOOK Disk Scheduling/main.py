...
# if DIRECTION = 0, then the algorithm will scan on the left side firstly until reach the end request at left side
# if it doesn't finish
# the requests then go to the right side  and don't take any request until reach the last request at right
# then continue take requests from right to left
...
def C_LOOK():
    ...
    if DIRECTION == 0:
        ...
        if len(requests) != 0:
            path_length += right - left - 1

        for p in reversed(range(left + 1, right + 1)):
            if len(requests) == 0:
                break
            index = binary_search(requests, p)
            path_length += 1
